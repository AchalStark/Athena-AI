/**
 * Athena AI - Advanced AI Assistant
...
class AthenaAI {
    ...
    async init() {
        try {
            ...
            console.log(`Athena AI v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize Athena AI:', error);
    ...
    setupEventListeners() {
        try {
            ...
            // Starter buttons in welcome screen
    ...
    displayMessage(message) {
        try {
            ...
            const avatarIcon = { user: 'user', assistant: 'bot', system: 'settings', tool: 'wrench' }[message.role] || 'message-circle';
    ...
    showWelcomeScreen() { 
        ...
        if (this.elements.chatArea) this.elements.chatArea.style.display = 'none';
    }
    hideWelcomeScreen() { 
        if (this.elements.welcomeScreen) this.elements.welcomeScreen.style.display = 'none'; 
        if (this.elements.chatArea) this.elements.chatArea.style.display = 'flex';
    }
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to Athena AI! How can I assist you?');
        }
    }
    ...
    showToast(type, title, message) {
        try {
            ...
            toast.innerHTML = `<div class="toast-icon"><i data-lucide="${icon}"></i></div><div class="toast-content"><div class="toast-title">${this.escapeHtml(title)}</div><div class="toast-message">${this.escapeHtml(message)}</div></div><button class="toast-close" aria-label="Close"><i data-lucide="x"></i></button>`;
    ...
    updateCharCount() {
        try {
            ...
            if (input && counter) {
                ...
                const maxLength = parseInt(input.getAttribute('maxlength')) || 5000;
